...
const FORGET = 0.15;
const EDGE_DECAY = { mentions: 1.0, related_to: 0.985, similar_to: 0.97, default: 0.99 }; // multiplicative per run
...
// ---- INGEST -----------------------------------------------------------------
async function ingestHarness(db, file, prune) {
...
function dreamCore(db, flags) {
  const now = new Date(Date.now() + (Number(flags["advance-days"]) || 0) * 86400000);
  ...
  const result = { runId, summary, facts: final, target: ENTRY_TARGET, max: ENTRY_MAX, status: after.status, pressure: after.pressure, evaporated_episodic: evap, evaporated_semantic: evapSem, reactivated: reentered.size, promoted_semantic: promoSem };
...
async function weave(db, opts) {
  ...
  return { new_entity_hubs: newHubs, mention_edges: mentionEdges, related_edges: relatedEdges, similar_edges: similarEdges, rescued_islands: rescued, remaining_islands: islands.length };
}
...
async function main() {
  ...
  try {
    ...
    else if (cmd === "ingest-harness") { r = await ingestHarness(db, flags.file, flags.prune === true || flags.prune === "true"); gate = !r.complete; }
    else if (cmd === "verify-sync") { r = verifySync(db, flags.file); gate = !r.complete; }
    else if (cmd === "dream") r = dreamCore(db, flags);
    else if (cmd === "weave") r = await weave(db, { k: Number(flags.k) || 3, sim: Number(flags.sim) || 0.45 });
