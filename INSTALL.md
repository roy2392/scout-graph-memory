...
2. **Install + bootstrap** (from inside the unzipped folder):
   ...
   npm run setup
   ```
   This builds the native deps, creates `~/.agent-memory/memory.db`, and warms the embedding model.

3. **Make the two skills visible to Scout.** Scout loads skills from your skills folder (e.g.
   `~/.copilot/m-skills/` on this machine — adjust to your install). Copy or symlink the two skill
   folders there:
...
4. **Point the skills at the engine.** Both `SKILL.md` files refer to the engine as
   `node <AGENT_MEMORY>/lib/...`. Set `<AGENT_MEMORY>` to wherever you unzipped in step 1 — either
   replace the placeholder in the two `SKILL.md` files, or (cleaner) export an env var your agent
   inherits:
   ```bash
   # add to your shell profile / Scout launch environment
   export AGENT_MEMORY=~/agent-memory          # the unzipped package dir
   # optional: co-locate the store with Scout's data dir
   export AGENT_MEMORY_DIR=~/.copilot/data
   ```

5. **Verify.**
   ```bash
   node lib/dream.js stats
   node lib/recall.js --query "test"
   ```
   `stats` should print node/edge counts; `recall` should return JSON (empty cluster until you ingest).
